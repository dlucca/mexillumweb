...
(async()=>{
 ...
 await page.locator('[data-install-field=afterhours][value=servidores]').check();
 await page.locator('[data-install-field=criticalLoads][value=frio]').check();
 await page.locator('[data-install-field=respaldoTiempo]').selectOption('corto');
 await page.locator('[name=sector]').selectOption('Bombeo');await page.locator('[data-install-field=variadores]').selectOption('todas');
 await page.locator('[name=sector]').selectOption('Institución educativa');
 if(await page.locator('[data-install-field=respaldoTiempo]').inputValue()!=='corto')throw Error('Sector switch lost university details');
 if(await page.locator('[name=days]').inputValue()!=='lv')throw Error('Sector switch lost the schedule answer');
 await page.screenshot({path:'.codex_tmp/onboarding-ui/03-operation.png',fullPage:true});
 await page.locator('[data-nav=map]').click();await page.getByRole('heading',{name:'Ubica tu instalación y los espacios disponibles'}).waitFor();await page.locator('[data-nav=summary]').click();await page.getByRole('heading',{name:'Tu consumo y las opciones de ahorro'}).waitFor();
 await page.locator('.exp-operation-summary summary',{hasText:'Datos de operación'}).click();await page.getByText('1 a 4 horas',{exact:false}).first().waitFor();
 ...
 await page.reload();await page.getByRole('heading',{name:'Tu consumo y las opciones de ahorro'}).waitFor();
 await page.locator('[data-action=submit]').click();await page.locator('.exp-success',{hasText:'Solicitud enviada.'}).waitFor({state:'attached'});
 const mobile=await browser.newPage({viewport:{width:390,height:844},isMobile:true,hasTouch:true});await mobile.route('https://maps.googleapis.com/**',r=>r.abort());await mobile.goto(link);await mobile.getByRole('heading',{name:'Tu consumo y las opciones de ahorro'}).waitFor();await mobile.screenshot({path:'.codex_tmp/onboarding-ui/05-mobile.png',fullPage:true});
 await mobile.locator('.exp-nav [data-nav=receipts]').click();await mobile.getByRole('heading',{name:'Empecemos por tus recibos'}).waitFor();await mobile.screenshot({path:'.codex_tmp/onboarding-ui/06-mobile-receipts.png',fullPage:true});
 ...
 await mobile.locator('[data-install-field=criticalLoads][value=ninguna]').check();if(await mobile.locator('[data-install-field=respaldoTiempo]').count())throw Error('Exclusive critical-load option did not hide followup');
 await mobile.locator('[data-install-field=criticalLoads][value=frio]').check();if(await mobile.locator('[data-install-field=respaldoTiempo]').inputValue()!=='corto')throw Error('Conditional field erased on hide');
