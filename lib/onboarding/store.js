import { sanitizeInstallations } from '../../js/expediente.installations.js';
...
export async function create(body,advisor) {
  ...
  const data={step:'receipts',contact,site:advisor?clean(body.site,200):'',address:advisor?clean(body.address,500):'',answers:advisor?sanitizeAnswers(body.answers):{},files:[],receipts:[],service:'',roof:null,location:null,servicePoint:null,consent:false};
...
export function publicRow(row) {return {id:row.id,revision:row.revision,data:row.data,expiresAt:row.expires_at,submittedAt:row.submitted_at||null,extractionEnabled:!!process.env.OPENAI_API_KEY};}
export async function storage(path,options={}) {
  ...
  if(!r.ok) throw fail(502,'No pudimos acceder al archivo. Intenta de nuevo.');
  return r;
}
