...
export async function extractReceipts(file,fileURL) {
  ...
  try {
        ...
        try {
          ...
          const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},
            body:JSON.stringify({model,store:false,service_tier:'default',reasoning:{effort:'low'},
              ...
              text:{format:{type:'json_schema',name:'cfe_receipts',strict:true,schema:receiptSchema}},max_output_tokens:4096}),signal});
    ...
    return {receipts:receipts.map((r,i)=>normalizeReceipt(r,file.id,i)),notes:results.flatMap(r=>r.notes).slice(0,15),usage:combinedUsage(usages,model)};
  }catch(e) {
